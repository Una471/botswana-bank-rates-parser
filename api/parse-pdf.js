    ...
    try {
        ...
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${GEMINI_API_KEY}`, {
            ...
            body: JSON.stringify({
                contents: [{
                    parts: [{
                        text: `You are a bank rate extraction expert. Extract ALL interest rates from this bank document and return ONLY valid JSON (no markdown, no explanation).

Document text:
${text.substring(0, 8000)}

Return this exact JSON structure (use null for missing values, numbers only for rates):

{
  "Bank Name": "exact bank name from document",
  "Report Period": "January 2026",
  "MoPR": 3.50,
  "Prime Lending Rate": 7.01,
  "Call Account Min": 0.00,
  "Call Account Max": 3.35,
  "Savings Min": 0.00,
  "Savings Max": 2.50,
  "FD 3M Nominal Min": 4.05,
  "FD 3M Nominal Max": 4.35,
  "FD 6M Nominal Min": 4.85,
  "FD 6M Nominal Max": 5.35,
  "FD 12M Nominal Min": 5.55,
  "FD 12M Nominal Max": 6.25,
  "FD 24M Nominal Min": 5.65,
  "FD 24M Nominal Max": 6.60,
  "Mortgage Rate Min": "Prime +12%",
  "Mortgage Rate Max": "Prime +17%",
  "Credit Card Rate Min": 22,
  "Credit Card Rate Max": 28,
  "Personal Loan Min": "Prime +17%",
  "Personal Loan Max": "Prime +22%",
  "Contact Phone": "316 8900",
  "Website": "https://fnb.co.bw"
}

Extract ALL rates you can find. If rate appears once, use same for min/max. Return ONLY the JSON.`
                    }]
                }],
                generationConfig: {
                    temperature: 0.1,
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const result = await updateAirtable(extractedData);
        
        res.status(200).json({ 
            success: true, 
            message: `✅ ${result.action} ${result.bank}`,
            data: extractedData
        });
